...
{
    public class Dialog_SelectKindsForHediffApply : Window
    {
        ...
        public Dialog_SelectKindsForHediffApply(FactionDef faction, KindGearData source, List<PawnKindDef> targets)
        {
        ...
        public override void DoWindowContents(Rect inRect)
        {
            ...
            Widgets.Label(new Rect(inRect.x, inRect.y, inRect.width, 30f), LanguageManager.Get("ApplyHediffsToOthersTitle"));
            ...
            if (Widgets.ButtonText(applyRect, LanguageManager.Get("Apply")))
            {
                if (selectedKinds.Count > 0)
                {
                    Find.WindowStack.Add(new Dialog_MessageBox(
                        string.Format(LanguageManager.Get("ApplyHediffsConfirm"), selectedKinds.Count),
        ...
        private void Apply()
        {
            ...
            {
                // Copy hediffs from source to target
                if (sourceKindData.ForcedHediffs != null)
                {
                    if (targetKindData.ForcedHediffs == null)
                        targetKindData.ForcedHediffs = new List<ForcedHediff>();
                    
                    // Deep copy hediffs
                    foreach (var hediff in sourceKindData.ForcedHediffs)
                    {
                        var newHediff = new ForcedHediff
                        {
                            HediffDef = hediff.HediffDef,
                            PoolType = hediff.PoolType,
                            maxParts = hediff.maxParts,
                            maxPartsRange = hediff.maxPartsRange,
                            chance = hediff.chance,
                            severityRange = hediff.severityRange,
                            parts = hediff.parts?.ToList()
                        };
                        targetKindData.ForcedHediffs.Add(newHediff);
                    }
            ...
            Messages.Message(string.Format(LanguageManager.Get("HediffsApplied"), count), MessageTypeDefOf.PositiveEvent, false);
        }
    }
}
