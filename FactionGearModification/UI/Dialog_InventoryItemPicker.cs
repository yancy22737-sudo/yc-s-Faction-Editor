...
{
    public class Dialog_InventoryItemPicker : Window
    {
        ...
        public override void DoWindowContents(Rect inRect)
        {
            ...
            Widgets.Label(new Rect(0f, y, inRect.width, 35f), multiSelect ? LanguageManager.Get("InventoryItemPickerTitle") : LanguageManager.Get("SelectItem"));
            ...
            if (multiSelect)
            {
                DrawBottomButtonsMulti(inRect);
            }
            else
            {
                DrawBottomButtonsSingle(inRect);
            }
        }
 
        private List<string> GetSortOptions()
        {
            return new List<string> { "Name", "MarketValue", "TechLevel", "ModSource" };
        }
        ...
        private void AddSelectedToTarget()
        {
            ...
            Messages.Message(LanguageManager.Get("InventoryItemsAddedMessage", added, skipped), MessageTypeDefOf.PositiveEvent, false);
            selected.Clear();
        }
        ...
        private static bool MatchesCategory(ThingDef def, ItemCategoryFilter? category)
        {
            ...
            switch (category.Value)
            {
                ...
                case ItemCategoryFilter.SocialDrug:
                    return def.IsDrug && !def.IsMedicine && def.ingestible != null && def.ingestible.drugCategory == DrugCategory.Social;
