...
namespace FactionGearCustomizer
{
    public static class GearApplier
    {
        ...
        // Hediff 安全阈值常量
        private const float IncapacitatingCapOffsetThreshold = -0.5f;
        private const float DangerousHediffSeverityThreshold = 0.5f;
        private const float LethalSeveritySafetyFactor = 0.95f;
        private const float ArmorRatingThreshold = 0.4f;
        private const float SocialDrugAddictivenessThreshold = 0.1f;
        ...
        private static bool WillHediffIncapacitatePawn(Pawn pawn, Hediff newHediff)
        {
                        ...
                        if (stage?.capMods != null)
                        {
                            foreach (var cap in stage.capMods)
                            {
                                ...
                                if (cap.capacity != null)
                                {
                                    ...
                                    if ((capName == "consciousness" || capName == "moving") && cap.offset <= IncapacitatingCapOffsetThreshold)
                                    {
                                        return true;
                                    }
                                }
            ...
            try
            {
                ...
                {
                    // 如果严重程度超过 0.5，认为太危险
                    if (newHediff.Severity > DangerousHediffSeverityThreshold)
                    {
                        return true;
                    }
                }
        ...
        private static List<BodyPartRecord> GetValidBodyPartsForHediff(Pawn pawn, HediffDef def)
        {
            ...
            foreach (BodyPartRecord part in allParts)
            {
                ...
                {
                    Log.Warning($"[FactionGearCustomizer] GetValidBodyPartsForHediff: skipped body part for hediff {def?.defName ?? "null"} on pawn {pawn?.Name?.ToStringFull ?? "null"}: {ex.Message}");
                }
            }
        ...
        private static void ApplyHediffSeverity(Hediff hediff, ForcedHediff forcedHediff)
        {
            ...
            {
                severity = Mathf.Min(severity, hediff.def.lethalSeverity * LethalSeveritySafetyFactor);
            }
        ...
        private static bool IsArmorMiddleCandidate(ThingDef def)
        {
            return CoversBodyPart(def, BodyPartGroupDefOf.Torso)
                ...
                && def.GetStatValueAbstract(StatDefOf.ArmorRating_Sharp) > ArmorRatingThreshold;
        }
        ...
        private static ThingDef GetRandomSocialDrugItem()
        {
            var socialDrugDefs = DefDatabase<ThingDef>.AllDefsListForReading
                ...
                .Where(x => x.DrugProps != null && x.DrugProps.addictiveness < SocialDrugAddictivenessThreshold)
                .Select(x => x.Def)
                .ToList();
